...
export const GET: RequestHandler = async ({ request, url }) => {
	...
	try {
		// Search for documents in the shared drive
		const params = new URLSearchParams({
			q: `mimeType = '${DOCS_MIME_TYPE}' and trashed = false`,
		...
		if (response.status === 401) {
			...
			try {
				...
				if (!retryResponse.ok) {
					...
					throw error(retryResponse.status, `Docs API error: ${errorText}`);
				}
		...
		if (!response.ok) {
			...
			throw error(response.status, `Docs API error: ${errorText}`);
		}
		...
		throw error(500, 'Failed to fetch documents from Google Docs');
	}
};
